...
### Changed
- **Product Search settings moved to a dedicated page + operator can hide the menu button (2026-08-04).** The Product Search URL row lived under Settings → System → Features; it now has its own **Settings → System → Products** page (only visible when the products overlay is on). The URL row's description is now the plain-English "Enter the URL to your product search/product data importer tool. Useful for quick navigation." — no more oblique companion talk. New toggle **Hide menu button** suppresses the "Product Search" main-nav entry entirely, independent of whether a URL is set — for operators who don't run any external search tool at all and don't want the entry advertising a feature they'll never wire up. Backed by a new `AppSetting.product_search_hidden` bool (default off, so behaviour is unchanged for existing installs). Existing bookmarks / in-app "set it up" redirects now land on `/settings/admin/system/products`.
