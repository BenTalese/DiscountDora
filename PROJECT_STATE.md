...
of the batch-pool model on the way. Suites are at their highest: backend **2265
...
| **Shared component anatomy** | 🟡 | 09-03 collapsed **four looks of one control** into the pill anatomy (a squared Quasar block, Reports' hardcoded `color="grey"` pair, the stock filter panel's hand-painted variant, settings' sunken fill); the opt-in `pill` prop is gone — *a shape prop with a clear winner is an unfinished migration* — and `--seg-*` tokens now stop the two drawing components drifting. **FU-848** (merging `BaseSegmented` and `DoraSegmented`) is its own unit: 19 consumers pass Quasar props that would go inert | `BaseSegmented.vue` · `settings/DoraSegmented.vue` |
...
| **Cook mode** | ➗ | Two batches (08-28 rebuild, 09-01 eleven items), both driven live. Structured steps now *record* `timer_minutes` instead of a regex sniffing prose for one (**R-068 / ADR-065**); the finish modal is one segmented control defaulting to "unchanged" (it used to write a level drop onto every ingredient of every cook); the three panels share one row treatment; header rebuilt after `--space-sm`, a token that never existed, zeroed every gap. Unreported bugs found: image mode could never finish a cook. Owed: FU-765, FU-763, two device-only checks | worklog 2026-09-01 · `RecipeCookMode.vue` |
