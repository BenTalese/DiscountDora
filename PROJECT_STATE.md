...
**Last reviewed: 2026-08-16.** Milestone-progress front door — phase board, workstreams, and what needs your attention. This is *not* a changelog; shipped-work history lives in `CHANGELOG.md` + `DORA_WORKLOG.md`.
...
| Cookbook | ➗ | Chunks 1–10 shipped; recipe-detail residuals; complex-mode nutrition on the card + kcal filter/sort in both modes (FU-637 done); FU-638 grid-render finding open | [PROPOSAL](docs/04_proposals/PROPOSAL_COOKBOOK.md) |
| Stock Overview | ✅ | 3-band StockLevel, buy-verdict badge, Needs-check filter, overdue pulse | `PROPOSAL_STOCK_OVERVIEW` |
| Stock-item detail | ➗ | 2026-08-16 feedback batch shipped (4 bugs incl. the QR breakage; collapsed Dora-thinks + buy-verdict cards; nutrition Details table; preferred-buys hidden under Products). **Browser-verify owed**; FU-648 (QR root cause never reproduced) is the one to close deliberately | `DORA_VERIFY` → Stock-item detail |
...
**Total open backlog is 33 items in `DORA_FOLLOWUPS.md`** (counted
