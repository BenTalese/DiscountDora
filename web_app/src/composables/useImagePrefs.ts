// C-cross Chunk 5 — recipe-image display opt-in. ADR-005 family
// composable: owns the per-user `show_recipe_images` flag AND the
// optimistic-flip setter that PATCHes /me. The recipes overview inline
// toggle button calls the setter; other surfaces just read
// `showRecipeImages` as a render gate.
//
...
// explicitly.
//
// FU-508 dropped the stock-image companion (`showStockImages` +
// `setStockImages`). Pantry items no longer carry their own photo —
// linked Products supply the visual instead.
