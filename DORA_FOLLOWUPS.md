...
## [OPEN] FU-548 — `POST /api/auth/logout` produces no audit event (the one un-audited mutating endpoint) — security judgment call
- **Raised:** 2026-07-12 (surfaced by the new FU-538 audit-coverage sweep).
...
## [OPEN] FU-546 — Filter/sort field resolution is a soft allowlist: `validate_known_fields` is dead code; non-column public attributes still 500
- **Raised:** 2026-07-12 (found by the new `test_security_injection.py` FU-537 suite).
...
## [OPEN] FU-540 — No automated browser / integration E2E layer (Playwright); everything above the component is manual (DORA_VERIFY)
- **Raised:** 2026-07-12 (test-suite deep-dive — missing test *type*).
...
## [OPEN] FU-536 — Alembic migration round-trip + portability tests (117 migrations, zero tests)
- **Raised:** 2026-07-12 (test-suite deep-dive). **Stub:** [tests/test_migrations.py](tests/test_migrations.py).
...
## [OPEN] FU-535 — Concurrency / race / idempotency tests (double-submit, lost update, reconcile-vs-verb, offline replay)
- **Raised:** 2026-07-12 (test-suite deep-dive). **Stub:** [tests/e2e/dora_api/test_concurrency.py](tests/e2e/dora_api/test_concurrency.py).
