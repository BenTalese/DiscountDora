...
1. Reassign any StockItem currently pointing at the Sufficient row
   (sequence=1) to the Well-Stocked row (sequence=0). StockLevelChange
   history keeps its denormalised `stock_level_name`, so audit trail
   survives — see StockLevelChange docstring.
2. Delete the Sufficient row (sequence=1).
3. Reseries: Low (was seq=2) → seq=1, Out (was seq=3) → seq=2. No unique
   constraint on `sequence` so no collision risk during the two-step
   update.
4. Rename the seq=0 row from "Well-Stocked" to "Stocked". Domain enum
   member also renamed WELL_STOCKED → STOCKED in the same commit.

Pre-release, no data preservation shim. Merges the three open heads
(product-unique / drop-nutrition / buy-verdict-enabled) that were
sitting alongside each other on 2026-07-02.
...
def upgrade():
    ...
    # 2. Delete Sufficient row. StockLevelChange FK is SET NULL on
    #    delete and keeps the denormalised name, so history survives.
    conn.exec_driver_sql("""
        DELETE FROM "StockLevel" WHERE sequence = 1
    """)

    # 3. Reseries Low (2 → 1) then Out (3 → 2). Rows are disjoint;
...
def downgrade():
    ...
    # distinction is lost by design). History remains intact via
    # StockLevelChange's denormalised name column.
    conn.exec_driver_sql("""
