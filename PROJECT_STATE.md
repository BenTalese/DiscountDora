...
| Recipe page | 🟡 | The redesign (`RecipeDetailNext.vue`) **won the owner comparison**, its gaps are closed (explicit Save + dirty indicator + route guard, ADR-045/R-049), and the **2026-08-24 feedback batch (15 items) is shipped and driven live**: Fraunces deleted (new **D-022** — no page-specific typeface), cost breakdown promoted to a proper modal, ingredient sections rebuilt as cards (the "Organise ingredients" panel gone), one shared pencil for all three step styles with a full-screen editor on phones, the structured step editor rebuilt (grip-free on mobile, up/down/sub-step/hint/remove), shopping-list membership surfaced in the Right-now cell *and* the picker's initial selection, substitutes surfaced on the row + the cook guard, and "Other versions" replaced by **Version information** on the back of a new `Recipe.updated_at` (migration `b3f7c1d9a2e6`). **Blocked on one owner call (FU-688)**: masthead vs shared `PageToolbar`, which gates the one-way swap pass (delete the 2,727-line old page — now two feedback batches behind) | [PROPOSAL](docs/04_proposals/PROPOSAL_COOKBOOK.md) |
...
| Test suite | ✅ | Green 2026-08-24: backend **1950 passed** / 1 skipped / 1 xfailed, frontend **494 passed**. Note for this machine: bare `pytest` throws ~56 spurious setup errors — pass `--basetemp=<real dir>`. Remnant: Postgres CI (FU-520, waits on FU-405) | [PROPOSAL](docs/04_proposals/PROPOSAL_TEST_SUITE_IMPROVEMENTS.md) |
