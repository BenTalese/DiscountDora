...
- **FU-317 Chunk 2 shipped — `Recipe.available_meals` mutation collapsed to one authority (2026-07-09).** Backend-only refactor, no behaviour change. New `dora_api/features/recipes/pool.py` with `bump_pool(recipe_id, delta, connection=None) -> int` (atomic `UPDATE ... RETURNING`, floor at zero, dual-mode ORM/connection) + `preview_pool_after` for the assistant's preview path. Three writers rewired — the sweep uses `bump_pool(_RecipeId, -_ToDecrement, connection=_Conn)` in its decoupled transaction; `cook_recipe` + `adjust_recipe_meals` use the ORM-session form and return the raw-SQL return value. `grep -rE '\.available_meals\s*=[^=]' dora_api/` now returns only Recipe-creation + dev-seed sites; the floor rule lives in one CASE WHEN. Closes the R-003 drift called out in `PROPOSAL_MEAL_RECONCILE.md` §5.1. Next: Chunk 3 — reconcile queue + per-entry verb endpoints.
- **FU-317 Chunk 1 shipped — receipt table + auto-drain setting + sweep rewritten (2026-07-09).** Backend-only, no user-visible change. Migration `a1b7f3e9c2d4` adds `AppSetting.auto_drain_past_meals` (default TRUE — matches today's behaviour) + creates `MealPlanReconcileReceipt` (append-only audit table, `ON DELETE CASCADE` to `MealPlanEntry`, six state values). `reconcile_consumed_meals` rewritten to read both `timezone` + `auto_drain_past_meals` in one connection round-trip, then branch: TRUE keeps historic auto-drain + writes an `unresolved_auto` receipt per drained entry; FALSE leaves the pool + entries untouched and writes `unresolved_manual` receipts only (idempotent via `NOT EXISTS` receipt-check). D5 (FU-517) resolved install-wide. `GET/PATCH /api/app-settings` extended for the new field. New e2e test file [`test_reconcile_receipts.py`](tests/e2e/dora_api/test_reconcile_receipts.py) covers both branches + idempotence + cascade. Next: Chunk 2 — collapse the two `Recipe.available_meals` write-sites into one shared helper (R-003 payoff).
- **FU-317 done — Proposal + impl-plan for manual meal-plan reconcile (2026-07-09).** [PROPOSAL_MEAL_RECONCILE.md](docs/04_proposals/PROPOSAL_MEAL_RECONCILE.md) locks the shape (modelled on `PROPOSAL_STOCKTAKE_MODE.md`): per-user `auto_drain_past_meals` setting (default on); new `/meal-plans/reconcile` surface with a server-owned queue + five per-entry verbs (Cooked / Cooked (different portions) / Didn't cook / Cooked later / Skip); new `MealPlanReconcileReceipt` append-only audit table; new `meal_reconcile_overdue` alert kind alongside the existing `no_planned_meals` (forward-looking) alert. The "auto-drain vs manual — mutually exclusive?" question refused as a false choice — they share the same page and verbs, the setting only decides an entry's *initial* state on rollover. [IMPL_PLAN_MEAL_RECONCILE.md](docs/04_proposals/IMPL_PLAN_MEAL_RECONCILE.md) breaks it into six chunks (schema + receipts → shared pool helper → queue+verb endpoints → alert+suggestion → page+chips → settings+copy) and answers the four residuals: D1 alert fires at ≥3 unresolved entries stretching ≥4 days back; D2 `cook_recipe` stays independent (no pool coupling); D3 receipts kept forever; D4 setting copy pinned as *"Assume I cooked past-day meals"* + explainer. Feedback coverage table at proposal §12 anchors L89 + L369 + L371. **No code yet** — next step is a session picking up Chunk 1 (schema + receipt writer, backend-only, no user-visible change) before any code touches `reconcile_consumed_meals.py` or the `startup.py:150` hook.
- **FU-360 #3 done — Basic/AI mode chip → toggle slider on the Dora chat header (2026-07-09).** New `DoraModeSlider.vue`: two-position pill, skewed knob that slides between "Basic" and "AI" with a brand-primary glow on the active side and a rem-based font size that follows the text-size preference. Tap flips the user's `llm_enabled` via `PATCH /auth/me`, then re-probes `/assistant/status` so `aiActive` + the "AI mode unavailable" banner update. `canToggleMode` mirrors AssistantSettings' `canEnable` (install master on + provider + model + base_url|api_key) so the disabled state carries an explanatory tooltip. Related: FU-383 (preferred-stores onboarding step) moved to `_RESOLVED` as won't-do — the existing `usual_store_id` server-side concept covers it. FU-360 remaining tail is #1 (text-size honoured) + #4 (DS4 hover-flash) browser verifies only. `vue-tsc` clean.
- **FU-407 / RD-18 done — Substitute-swap on shopping lists only offers when there's a substitute (2026-07-09).** The per-line "Swap with substitute" button used to be a dead-end for items with no recorded substitute (tap → "No substitutes recorded" toast). Lines now carry a server-derived `has_substitutes` flag, so the action is disabled up front with a clear tooltip. The relabel also disambiguates it from the "store offers" picker on the same line.
