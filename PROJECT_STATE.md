...
**Last reviewed: 2026-09-12 (stocktake belief copy, then a ~20-item meal-planner batch whose headline was the builder pricing the pot instead of the plan).** Milestone-progress front door — phase board,
...
| **1 — Close the loop** | Shopping lists, cook mode, stock, cookbook, meal planner, suggestions, costing, stocktake | ➗ ~97% | **Nothing is unbuilt. Corrected 2026-09-07:** this row claimed meal-reconcile Chunk 6 (settings row/copy) was the last unbuilt item. It was built in an earlier feedback batch (commit `850c04ae`) and the row was never updated — `AdminSystemMealReconcileSettings.vue` exists, is routed at `/settings/admin/system/meal-reconcile`, sits in the settings nav, and meets the Chunk-6 spec (admin-gated toggle on `auto_drain_past_meals`, household-voice explainer, link through to the reconcile page). **All remaining Phase 1 work is browser-verify.** Everything else is browser-verify, and 09-05 added a third layer of it: the whole thirteen-item planner batch (rail disclosure + counts, the slot-sectioned phone day, the surviving servings menu, cost on strip/day/card/rail), on top of the `cook_fresh` batch-household path, the dashboard's batch-household and manual-reconcile states and its money-off/products-off card gates (FU-868). Still owed from before: stock overview (08-20/21/22), the stocktake three-phase runner, cookbook batch 3, the older recipe-page batches, the buy-verdict money gate, `planned_store_id` round trip. |
| **2 — Ingestion API + companion** | `/api/ingest` seam, standalone companion, Merchant→Store | ➗ reopened and largely rebuilt 2026-09-06 | The products area was reopened by the owner and re-planned as a batch program across both repos ([IMPL_PLAN_PRODUCTS_PROGRAM](docs/04_proposals/IMPL_PLAN_PRODUCTS_PROGRAM.md)); **6 of 8 batches shipped the same day** — see the workstream row below. L197 hard-delete is **built**; FU-881/882/883/885 and **FU-214** are resolved. Remaining here: **FU-884** (Aldi provider defects + rewrite, batch E, blocked on OD-3). Unchanged and still open: **FU-856** (`pack_count` unreachable through the products API, so R-076's better cost answer stays out of reach), **FU-866** (the ingest route never checks its own flag). |
...
|---|---|---|---|
| **Meal planner** | 🟡 | **A ~20-item owner batch landed 09-12**, three items of which were real defects rather than styling: the auto builder quoted each proposed meal at the **whole recipe's** cost (so the step-1 servings control changed nothing and a 3-day batch cook was counted three times — and the budget cap was swapping meals on that figure); **Reshuffle** varied a plan only when two recipes scored exactly alike, which on a real cookbook never happens, and now samples among the near-best and rotates the days; and `margin-left: auto` on the card's status glyph right-aligned the servings/time/cost of only the meals that had one. Two new server-owned fields replaced client derivations (R-003): the ingredient aggregate now ships **low/out counts plus a to-buy and a meals cost**, and each planned entry ships **`missing_count`**, which is what lets a card flag "3 to buy" on meals that are actually cooked from ingredients and stay silent on a leftovers day. The builder's review step is cards-per-meal with a centred day divider, the phone picker names its destination in the sheet title (and lost the Done button that did nothing there), and the rail gained a **Can cook now** chip with the A-Z/Cheapest control folded into the chip row. Owner calls taken up front: budget basis stays meal-value **with the to-buy figure shown**; cookability flags the gap only. None of it walked yet. Open: FU-908, FU-907, FU-879, FU-878, FU-863, FU-862, FU-802 · Previously — a **13-item owner batch on 09-05**, on top of the ~35-item sweep and `cook_fresh`. The rail's "This week's shopping" became *Missing this week*, folded into the disclosure header with tinted out/low count circles and the cart beside it; both rail disclosures are now one hand-rolled `MealPlanRailDisclosure` because `q-expansion-item` structurally can't keep an action visible when collapsed. The two entry menus (`MealPlanEntryChip` / `MealPlanRichCard`) were **copy-paste identical** and became `MealPlanEntryMenu` (R-001). The phone's day is now a section per slot in the household's own order, every slot present with its own Add — which retired the "Which slot?" sheet. **Money reached the planner**: `_hydrate_cost` gives per-entry cost at planned servings, a per-day rollup, a week total with coverage counts, plus an A-Z/Cheapest rail ordering off the newly extracted `recipeCostSort.ts`. The servings-stepper bug was **`:key` churn** — `PATCH /meal-plans` replaces every forward entry, minting new ids (**R-086/ADR-083**; server-side churn deferred as FU-878). None of the batch, and none of `cook_fresh`, has been walked by the owner either. | [PROPOSAL_MEAL_PLANS](docs/04_proposals/PROPOSAL_MEAL_PLANS.md) · R-086 |
...
| **Products program (Dora + companion)** | ✅ complete | Delivered 2026-09-06 → 09-08, **all eight batches, all four decisions closed**. End-to-end Dora↔companion data path · **every June My Products + Price History bullet actioned (FU-214 closed)** · Aldi scraper rewritten for the site redesign (now with brand + stockcode, which is what makes refresh-by-stockcode work at all) · **custom products** for shops no scraper covers, strictly marked and excluded from sync and ingest dedupe. Established **R-087/ADR-084**. Decisions: OD-1 card actions · OD-2 custom products **built** · OD-3 don't add aggregator sources · OD-4 bearer key stays env-only | [PLAN](docs/04_proposals/IMPL_PLAN_PRODUCTS_PROGRAM.md) |
...
| **Recipes & cookbook** | ➗ | **R-076/ADR-073** gave one `_item_price` helper authority over "what does one of these cost?", allowed to answer *no*. 09-05 took cost to the cookbook (per-serving on rows and cards, a Cost-per-serving sort, two queries for the page, skipped outright with money off, plus a money-**on** query-count test because the existing one passed vacuously) — and the comparator behind that sort has now been extracted to `recipeCostSort.ts` and reused by the planner rail. The units investigation (`RECIPE_PRICE_UNIT_MISMATCH.md`) found the density bridge exists and is never called. **09-08 took a six-item owner batch on the recipe view**, whose one substantial item was the nutrition panel: the rollup now ships each uncounted ingredient *named* (detail path only), and the panel offers the pantry page's food picker inline on the two gaps that live on the stock item — so a partial calorie figure is now something you can close from where you read it, rather than a paragraph to nod at. The panel was rebuilt around it (headline + nutrient grid + the cost dialog's coverage bar); the batch also fixed the cost dialog's share bars, invisible in every dark theme. **09-09 took a five-item cookbook/recipe batch**, whose substantial half was the *other* end of the same problem: three defects were keeping counts and volumes out of the rollup entirely — `whole` wasn't a recognised unit (so "1 whole onion" weighed nothing while "1 onion" weighed 110 g), `stick` was 113 g **of butter** applied to celery, and the 77-entry density table matched pantry names only *exactly*, so it fired for `milk` and never for `Full Cream Milk`. All three fixed and pinned; all three measured before and after rather than reasoned. The rest of the batch was a cache and a lifetime bug: the rating chip needed a page reload because the scheme flip refreshed the flags but not the recipe DTOs that carry the server-computed rating, and the "uses expiring ingredients" filter came back from a recipe switched-on-but-inert because its chip is session state and its data is not. **FU-874 is resolved the same day**: `_line_cost` passes the ingredient name, the planner's hand-written cost adapter was fixed in step (it would have quoted a lower figure than the cookbook for one recipe), `stick` became one shared ambiguity record both calculators read, and an amount-less ingredient stopped being billed as a whole product. Found by driving one realistic 18-ingredient recipe line-by-line through both calculators rather than reading the code — which also produced **FU-904** (an unrecognised unit word is billed as a whole item: 3 cloves of garlic → $3.60) and **FU-905** (the two calculators can't see each other's data and disagree about a cup of flour by 6%). **FU-904 and FU-905 followed the same day**: sub-item words (`clove`, `sprig`, `rasher`, `knob`…) stopped being billed as whole products, and the two calculators stopped answering "how much of this is there?" from different data — the grams resolution is now one shared `household_measures` module stating *measured portion beats modelled density*, which both settles the cup-of-flour disagreement and lets costing weigh a counted ingredient ("2 leeks" @ $6.90/kg: unpriced → $1.23). Open: FU-895 (the three gap reasons still without an action), FU-902 (the picker can't offer the units the rollup weighs best), FU-901, FU-900, FU-875, FU-855, FU-856, FU-857, FU-876 | `recipe_cost.py` · [INVESTIGATION](docs/05_investigations/RECIPE_PRICE_UNIT_MISMATCH.md) |
...
| **Build & deploy** | 🟡 | The 08-23 backups/import 404 was the deploy script, not the app (`--exclude='data'` matched at any depth and `--delete`d `dora_api/features/data/`) — one-character fix, still owner-side. **09-07: the script now deploys the companion too** — that repo gained its first container story (`Dockerfile`/`compose.yml`/`nginx.conf`/`startup.sh`) as a second Compose stack on 5172/5175, and the deploy mints Dora's ingestion key itself so a wiping deploy re-keys the companion instead of muting it. FU-790's reclaim step is finally *wired* (it had never run). **None of it has touched a real daemon** — no Docker in-session; the first deploy is the proof (FU-892, DORA_VERIFY). FU-719, FU-720, FU-721 | FU-719/720/790/892 |
...
**The products program is COMPLETE and has nothing open** — all eight batches, all four decisions closed (2026-09-08). ⚠️ **One action for you:** the companion 403s until ingestion is switched on — Settings → Admin → Data & access.
...
4. 🟡 **FU-875 — the cost breakdown says "Units don't match the price" for four unrelated situations.** Unblocked: its stated blocker (FU-874) is resolved, so the bucket no longer mostly hides a known bug, and 09-09 added a fourth case it conflates (a food-specific unit on the wrong food). `no_quantity` landed the same day and is the shape this proposes, so the copy map and the closed union are already set up to take more. **Resolution: now-ish.**
...
21. 🟢 **Products-as-overlay's remaining browser-verify.** The three *product* surfaces (My Products, Price History, the stock-item Products tab) were driven on 2026-09-06 and **FU-214 is closed** — its four long-standing bullets are confirmed fixed. What was never walked is the rest of Phases 0–E (ingest admin, store mapping, quarantine). Entry point: the FU-210 tail.
...
| ENGINEERING_STANDARDS.md | Rules / ADR log | 🟢 authoritative | R-001..R-088 code rubric + ADR-001..085 log, checked every task | Living; R-088 + ADR-085 (app-wide component defaults live in a `Base*` wrapper) added 2026-09-09. Row previously read "R-001..R-040" — stale since August |
