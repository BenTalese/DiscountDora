...
- **What:** Build the surfaces in [`PROPOSAL_USAGE_TELEMETRY.md`](docs/04_proposals/PROPOSAL_USAGE_TELEMETRY.md). **Surface A is not a stats dashboard — it's an *efficiency lens*** (owner reframe): the admin opts in (off by default), and the app shows *interpreted, actionable* coaching over their own install — "do this faster this way," "sub-optimal order," "this user uses 5% of Dora," "least-used feature" — with the raw counts underneath. Self-host = **strictly local admin reporting, never egress**. **Surface B** (the traditional "share my usage data to improve the app" checkbox → anonymised aggregate egress to an author-run collector) is **hosted-only** — it does not exist on self-host, lives in the optional SaaS plan, and introduces the app's **first outbound behavioural egress** → no build without explicit owner sign-off on payload + collector + opt-in copy (proposal §4/§7).
- **Build shape (proposal §6):** admin opt-in gate → event registry + `usage_event_daily(user_id, event_key, day, count)` → instrument ~20–30 headline surfaces (incl. paired by-hand-vs-fast-way events) → **insight-rule engine** (curated ruleset, §3.2 catalogue) → Settings → Admin → Usage panel (Insights on top, counts underneath).
- **Owner-confirm-at-build decisions (proposal §7 items 4–5, recommendations recorded):** (4) include a **per-user** count dimension for per-user coaching — *recommended yes, counts-only, aggregate-about-a-user, never a per-action timeline* (most sensitive choice — confirm before build); (5) order/flow advice via **co-occurrence + curated rules first**, escalate to a capped local flow buffer only if too blunt — *no sequence stored by default*.
- **Why deferred:** design-only unit; the build competes with the rest of the backlog. Surface A needs the two §7 confirmations; Surface B needs the hosted context + sign-off.
- **Recommended resolution:** Surface A — opportunistic / when prioritised (confirm §7 items 4–5 first). Surface B — hosted-only, when the "across-installs" question is real **and** owner has signed off on §4 (tie to the OPTIONAL_SAAS revisit).
