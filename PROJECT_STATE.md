...
rendered four. **Chunk 5 closed the review**, and found the same class of bug one
last time: the cards had no *loading* state, so for the whole flight of a request
the page rendered its **empty** answers — "Nothing cooked in this range yet" to a
household with ten cooks. Every card now shows an outline of its own content
while it loads, empty states finally carry B9's icon, focused row links have a
ring (they computed `outline-style: none`), and the range picker names the dates
it means. The token half of that sweep was nearly empty by the time it ran, which
is the argument for sequencing it last.
...
| **Reports page** | ✅ | **`REPORTS_PAGE_REVIEW.md` is closed — all five chunks shipped 2026-09-02, every one driven live.** Chunk 1 gated the page — six endpoints 403 (R-058), six cards `v-if` off the money flag — while the count-based reports keep the nav entry. Chunk 2 took the defects: spend-by-store was answering a **different question from the receipt** (it now shares the shopping list’s `resolve_store_id` chokepoint); price trends **500’d on every bounded range** on SQLite; the product picker could only see 50 products; failed fetches rendered as *empty* states. **Chunk 3 restructured it: eleven cards → four** — Spend (store / group / vs-last-period on one axis control), Waste & run-outs, Kitchen memory, Price trends — plus a lede that opens with the answer, `<DashboardCard>` in place of the `.report-card*` fork (**FU-814 closed**), a shared `ProportionBar` in place of the donuts, stock-value-over-time cut, and §3.8’s repertoire count built. **Chunk 4 added the widget §5 called the largest one missing: Price changes** — `GET /reports/item-price-movers` ranks the household’s **own** stock items by per-unit price movement, refuses to compare across price dimensions or off a single reading, reports what it skipped (R-041), and opens each row into the shared chart. Money-gated, not products-gated: it is the price report an install with an empty catalogue can still have (FU-703 D3, whose keep/cut fork on the product surfaces is now better-informed but still open). **FU-833 landed with it: ECharts is gone** — `PriceHistoryChart` generalised to neutral series + an adapter module (**R-073 / ADR-070**), given the legend and the `role="img"` / `<desc>` text alternative §4.9 asked for, and adopted by all four chart consumers; **the Reports route chunk went 549 KB → 24 KB, measured**. **Seven defects across the four chunks were found by driving it or running the suite, not by reading it:** the gates were a *race* against `/api/health` (→ **FU-844**); "No store set" drew in a real store’s colour (→ **FU-843**, item 2 closed); the cook timeline emitted only the days you *did* cook, so a quiet month drew as an unbroken slab; the Spend card kept its coverage footnote under "I couldn’t load your spend"; chunk 3’s commit had three red backend unit tests; Price changes quoted "$24.00/L" for an item whose own chart said "$2.40/100ml"; and it counted three unmoved items as movers, so it claimed seven changes and rendered four. **Chunk 5 closed it, and its find was not a token:** three of five cards took no `loading` prop, so while their requests were in flight the page rendered their *empty* states — *“Nothing cooked in this range yet.”* on a household with ten cooks, *“log what you paid for a couple of items…”* to one that had. That is chunk 2’s failure-reads-as-absence bug on the other async edge, introduced by the restructure itself; every card now carries a skeleton shaped like its content. Also built: B9’s empty-state anatomy via a shared `CardEmpty` (the `.dora-empty` class could never supply the icon, so every consumer app-wide shipped the well without it), focus rings after a measurement found focused row links computing `outline-style: none`, a text alternative for the cook chart whose figures were hover-only, and the range control naming its window — which **deleted** the client’s copy of the server range table (R-003) rather than adding a field to nine responses. The token half was nearly empty by then (**zero** raw font-sizes, **zero** raw radii), which is the argument for sweeping last. Two regressions were fixed on the way in, one of them from the dashboard’s own chunk 6: the mascot paddings the owner had hand-tuned had been rounded onto `--space-1`, doubling one and quadrupling the other. Open: FU-843 item 1, FU-844, FU-845 #8, FU-846, FU-847 | [REVIEW](docs/05_investigations/REPORTS_PAGE_REVIEW.md) |
...
inferred* (`^## [OPEN] FU-` headings, 2026-09-02, after Reports chunk 5). **The previously-stated 190 was also wrong** — the same file measured 185
