...
**Last reviewed: 2026-08-20 (stock-signal consolidation — Step 0 answered, Chunk 3 landed).** Milestone-progress front door — phase board, workstreams, and what needs your attention. This is *not* a changelog; shipped-work history lives in `CHANGELOG.md` + `DORA_WORKLOG.md`.
...
| Stock Overview | 🟡 | 3-band StockLevel, Needs-check filter. **Signal consolidation in flight** — Chunks 1–3 landed: the buy verdict is off the row entirely (D-10), and **attention is now one server-owned rule** (`stock_attention.py`) that the outline, the count, the filter chip and the bell all read, closing B1–B4; the two-tier amber/red outline collapsed to one (D-7). The tapered Essential tab was reverted to a straight bar (2026-08-20). **Chunks 4–6 remain** — sort + row treatments, queue ranking, runner rebuild. Browser-verify owed | [IMPL_PLAN](docs/04_proposals/IMPL_PLAN_STOCK_SIGNAL_CONSOLIDATION.md) + `PROPOSAL_STOCK_OVERVIEW` |
...
00. **Walk the consolidated stock/alerts surface in a browser.** Step 0 is
   **answered** and **Chunk 3 has landed** (2026-08-20), so nothing in
   `IMPL_PLAN_STOCK_SIGNAL_CONSOLIDATION.md` is waiting on you any more — but the
   visual half is unseen. Worth 5 minutes: one outline instead of two, a quieter
   bell, and the row highlighting finally agreeing with the "Needs attention"
   count (they ran two different rules before, four documented ways apart).
   One thing to confirm rather than just check: **an overdue shopping day now
   counts on the bell badge**, a side effect of tier deriving from severity — a
   one-line reversal if you'd rather it stayed quiet. `DORA_VERIFY.md` → "Alerts +
   Stock: Step-0 cuts and the one attention rule". **Chunk 4** (sort + row
   treatments) is unblocked and is the natural next build. See FU-683.
