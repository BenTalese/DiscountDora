...
def search_for_product(search_term: str, start_page: int = 1) -> List[ScrapedProductOffer]:
    ...
    _WoolworthsProductOffers = [ScrapedProductOffer.translate_woolworths_offer(_ProductOffer)
                                for _ProductOffer
                                in woolworths_logic.search(_Session, search_term, start_page, start_page + 1)]

    _Session.get('https://www.coles.com.au/')
    _ColesProductOffers = [ScrapedProductOffer.translate_coles_offer(_ProductOffer)
                           for _ProductOffer
                           in coles_logic.search(_Session, search_term, start_page, start_page + 1)]
...
if __name__ == "__main__":
    x = search_for_product("brazil nuts")
