...
**Last reviewed: 2026-09-09 (five-item cookbook/recipe batch; nutrition can now weigh counts and volumes it used to skip).** Milestone-progress front door — phase board,
...
| **Products program (Dora + companion)** | ✅ complete | Agreed and delivered 2026-09-06 → 09-07, **all eight batches**. The Dora↔companion data path is end-to-end (save what you picked · receipts survive deletion · unsave · scheduled refresh), **every June My Products + Price History bullet is actioned (FU-214 closed)**, and the **Aldi scraper is rewritten** against the site's redesign — now with brand + stockcode, which is what makes refresh-by-stockcode work for Aldi at all. **Aldi shows no deals by design** (everyday-low pricing, no was-price). Established **R-087/ADR-084**. Answered: OD-1, OD-3 (don't add aggregator sources), OD-4 (key stays env-only). **Open: OD-2 — manual product entry**, a product decision, not work | [PLAN](docs/04_proposals/IMPL_PLAN_PRODUCTS_PROGRAM.md) |
...
**The products program is COMPLETE** (all eight batches, 2026-09-07). ⚠️ **One action for you:** the companion 403s until ingestion is switched on — Settings → Admin → Data & access. **One question left: OD-2 — should Dora allow manually-typed products?** Currently refused by design; see the plan's §6.
