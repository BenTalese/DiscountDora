<template>
    <q-select bg-color="white" class="min-width-200 q-ma-sm rounded-borders shadow-1" :clearable="clearable"
        clear-icon="cancel" dense color="black" square dropdown-icon="expand_more" :label="label" label-color="black"
        :multiple="multiple" :options="options" :option-label="optionLabel" standout="bg-teal"
        v-model="internalModelValue" @update:model-value="onUpdateModelValue">
        ...
        <template v-if="multiple" v-slot:option="scope">
            <q-item v-ripple clickable @click="scope.itemProps.onClick">
                <div class="items-center no-wrap row">
                    <q-icon size="md" class="q-pr-sm" :name="getOptionIconName(scope)" />
